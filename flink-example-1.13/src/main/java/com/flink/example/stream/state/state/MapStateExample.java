...
public class MapStateExample {
    ...
    public static class CountFunction extends RichMapFunction<SimpleUserBehavior, Tuple2<Long, Long>> {
        // 存储截止到当前的数量
        private MapState<Long, Long> countMapState;

        @Override
        public void open(Configuration parameters) throws Exception {
            // 计数状态
            MapStateDescriptor<Long, Long> uvStateDescriptor = new MapStateDescriptor<>("countMapState", Long.class, Long.class);
            countMapState = getRuntimeContext().getMapState(uvStateDescriptor);
        }

        @Override
        public Tuple2<Long, Long> map(SimpleUserBehavior behavior) throws Exception {
            Long userId = behavior.getUserId();
            long count = countMapState.get(userId) == null ? 1L : countMapState.get(userId) + 1;
            countMapState.put(userId, count);
