...
/**
 * 功能：基于 DataSketches HyperLogLog 实现 UV 历史累计去重 优化版本
 * 作者：SmartSi
 * CSDN博客：https://smartsi.blog.csdn.net/
 * 公众号：大数据生态
 * 日期：2022/10/27 下午11:38
 */
public class SketchesHyperLogLogUvV2 {

    private static final Logger LOG = LoggerFactory.getLogger(SketchesHyperLogLogUvV2.class);
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 本地去重
        DataStream<Double> localStream = source.keyBy(v -> ThreadLocalRandom.current().nextInt(3))
                .map(new HyperLogLogUV()).setParallelism(2);

        // 全局聚合
        DataStream<Double> globalStream = localStream.keyBy(v -> 1).reduce((v1,v2) -> v1+v2).setParallelism(1);

        // 输出
        globalStream.addSink(new PrintSinkFunction<>()).setParallelism(1);

        env.execute("SketchesHyperLogLogUvV2");
    }
